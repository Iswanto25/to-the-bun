...
- **S3 → Bun.S3Client**: Migrasi dari `@aws-sdk/client-s3` + `@aws-sdk/s3-request-presigner` ke `Bun.S3Client` bawaan.
- **Redis → Bun.RedisClient**: Migrasi dari `ioredis` ke `Bun.RedisClient` bawaan.
- **File IO**: `generateData.ts` menggunakan `Bun.write()` alih-alih `fs.writeFileSync()`.

### 🗑️ Removed

- `@aws-sdk/client-s3` — digantikan `Bun.S3Client`
- `@aws-sdk/s3-request-presigner` — presign sudah built-in di `Bun.S3Client`
- `ioredis` — digantikan `Bun.RedisClient`

### 📝 Note

Fitur yang **belum dimigrasi**:

- **BullMQ** → `Bun.cron` hanya untuk scheduled tasks, bukan job queue
