...
export const authController = {
	...
	deleteProfile: async (ctx: any) => {
		...
		if (!id) {
			return respons.error("Id tidak boleh kosong", "Id cannot empty", HttpStatus.BAD_REQUEST, ctx);
		}
		if (ctx.user.id !== id && ctx.user.roleName !== "Superadmin") {
			return respons.error("Anda tidak memiliki akses untuk menghapus profil ini", "Forbidden", HttpStatus.FORBIDDEN, ctx);
		}
